...
```{r}
## Deduplicated sample preparation

# Reading data
rawData_trimmed_dedup <- read.table('init_data/counts_matrix_trimmed_dedup.txt', 
                      header = TRUE, 
                      row.names = 1,
                      stringsAsFactors = FALSE)
...
# Check if there's any unwanted metadata row, e.g., 'Geneid', and remove it if necessary
# If the first row is metadata, you can drop it explicitly:
if ("Geneid" %in% rownames(rawData_trimmed_dedup)) {
  rawData_trimmed_dedup <- rawData_trimmed[-which(rownames(rawData_trimmed_dedup) == "Geneid"), ]
}

# Convert chr to int, preserving rownames - using [] to preserve row attributes
rawData_trimmed_dedup[] <- lapply(rawData_trimmed_dedup, as.integer)

# Check for any NA values (indicating conversion issues)
any_na <- any(is.na(rawData_trimmed_dedup))
# If any NA values are found, investigate
if (any_na) {
  print("Warning: Some non-numeric values were converted to NA. Please check your data.")
} else {
...
# Assign sample names
colnames(rawData_trimmed_dedup) <- c('KO1', 'KO2', 'KO3', 'WT1', 'WT2', 'WT3')

# Create DGEList object
rawDGE_td <- DGEList(rawData_trimmed_dedup)
...
# Add grouping to the DGEList object
rawDGE_td$samples$group <- group

# filtering genes by expression, getting rid of low count genes
keep <- filterByExpr(rawDGE_td)
rawDGE_t <- rawDGE_td[keep, , keep.lib.sizes=FALSE] # update lib sizes

# data normalization
rawDGE_td <- normLibSizes(rawDGE_td,
                              method = "TMM")
```
...
### Interactive Volcano Plot - for trimmed reads

```{r}
###############################
### Defining Interactive Volcano function
###############################
...
volcano_plot <- create_volcano_plot(DE_KOvsWT, 
                                    p_cutoff = 0.05, 
                                    fc_cutoff = 4.0, 
                                    x_breaks = 5,
...
### Interactive Volcano Plot - for trimmed + de-deuplicated reads

```{r}
# Releveling the group factor variable
rawDGE_td$samples$group <- relevel(rawDGE_td$samples$group, ref = "WT")

# Setting the desing 
design <- model.matrix(~group, data = rawDGE_td$samples)
colnames(design) <- c("WT", "MecrKO")

## EdgeR`s voomLmFit does all this under the hood: logCPM, limmaWeighted, corfit, lmFit
fit <- edgeR::voomLmFit(counts = rawDGE_td,
                        design = design,
                        sample.weights = TRUE
                        )
# edgeR::voomLmFit performs limma`s voomWithQualityWeights with duplicateCorrelation
...
```{r}
# Assuming your DE data is stored in `DE_KOvsWT`
volcano_plot <- create_volcano_plot(DE_KOvsWT_dedup, 
                                    p_cutoff = 0.05, 
                                    fc_cutoff = 4.0, 
                                    x_breaks = 5,
                                    color_pallette = c("gray", "forestgreen", "skyblue", "orange"))
...
# Convert to an interactive plot using plotly
interactive_volcano_plot_dedup <- ggplotly(volcano_plot, tooltip = "text")
...
# Print the interactive plot
interactive_volcano_plot_dedup

# Save the interactive plot as an HTML file if needed
htmlwidgets::saveWidget(interactive_volcano_plot_dedup, "volcano_plot_interactive_dedup.html")
```


###### t-statistic rankins and correlation for trimmed and trimmed+deduplicated samples

```{r fig.width=8, fig.height=6}
# Rank genes in each method
DE_KOvsWT$rank <- rank(-DE_KOvsWT$t)
DE_KOvsWT_dedup$rank <- rank(-DE_KOvsWT_dedup$t)

# Merge results
merged_ranks <- merge(DE_KOvsWT[, c("rank", "t")], 
                      DE_KOvsWT_dedup[, c("rank", "t")], 
                      by = 0, suffixes = c("_trimmed", "_trimmed_dedup"))

# Calculate Spearman correlation
cor_value <- cor(merged_ranks$rank_trimmed, merged_ranks$rank_trimmed_dedup, 
                 method = "spearman")

# Plot
ggplot(merged_ranks, aes(x = rank_trimmed, y = rank_trimmed_dedup)) +
  geom_point(alpha = 0.1) +
  geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
  labs(x = "Rank in Limma (t-statistic)", y = "Rank in LimmaW (t-statistic)",
       title = paste("Comparison of Gene Rankings (Spearman cor =", round(cor_value, 3), ")")) +
  theme_minimal()
```
Significant DE genes (p < 0.05) identified in the trimmed and trimmed+deduplicated counts data.

```{r fig.width=8, fig.height=8}
# Load the VennDiagram and grid libraries
library(VennDiagram)
library(grid)
...
sig_trm <- DE_KOvsWT[DE_KOvsWT$adj.P.Val < adj_pval_threshold, ]
sig_trm_dedup <- DE_KOvsWT_dedup[DE_KOvsWT_dedup$adj.P.Val < adj_pval_threshold, ]

# Get gene IDs (assuming rownames are gene IDs)
genes_trm <- rownames(sig_trm)
genes_trm_dedup <- rownames(sig_trm_dedup)

# Create a Venn diagram
venn.plot <- venn.diagram(
  x = list(
    Trimmed = genes_trm,
    Trimmed_dedup = genes_trm_dedup
  ),
  category.names = c("Trim", "Trim + Dedup"),
  fill = c("orange", "royalblue"),
  alpha = 0.5,
  cex = 2,
  cat.cex = 1.5,
...
# Clear the plot page and draw the Venn diagram
grid.newpage()
grid.draw(venn.plot)
```
...
## MSigDB Hallmark Pathways enrichment analysis
...
# Rank by t.value
ranked_genes_dedup <- DE_KOvsWT_dedup$t
names(ranked_genes_dedup) <- rownames(DE_KOvsWT_dedup) # Ensure Gene column contains gene symbols

# Sort genes by t (or other ranking criterion), just in case it hasn`t been sorted already
ranked_genes_dedup <- sort(ranked_genes_dedup, decreasing = TRUE)
...
# Perform GSEA using clusterProfiler
GSE_H_dedup <- GSEA(ranked_genes_dedup, 
                     TERM2GENE = msigdb_H[, c("gs_name", "gene_symbol")], 
                     pvalueCutoff = 1, 
                     verbose = FALSE,
                     pAdjustMethod = "fdr",
...
```
  ...
  >  
  ```
  Warning: There are ties in the preranked stats (52.54% of the list).
  The order of those tied genes will be arbitrary, which may produce unexpected results.
  ```

```{r fig.width=10, fig.height=8}
p <- custom_dotplot(gsea_obj = GSE_H_dedup, 
               showCategory = 50, 
               font.size = 10, 
               title = "Top MECR-KO GSEA: All hallmark pathways. Deduplicated counts",
               sortBy = "GeneRatio", 
               filterBy = "NES", 
               q_cut = 1,
               replace_ = TRUE, 
...
# Extract the data used for plotting
gsea_data <- as.data.frame(GSE_H_dedup)
...
# Save the plot to disk using ggsave()
ggsave(
  filename = "imgs/GSEA/Hallmark/GSEA_Hallmark_all_dedup.png",  # Specify the file path and name
  plot = p,  # The plot to save
  width = 10, height = 8,  # Dimensions of the image in inches
  dpi = 300  # Resolution of the image
)
...
## MSigDB C5 Gene Ontology Molecular Function Sets enrichment analysis
...
## MSigDB C2 Canonical Pathways (CP) gene sets enrichment analysis
...
## IMMUNESIGDB Pathways enrichment analysis
...
```{r include = FALSE}
## Final figures for the publication
...
# 	- dots larger 
# 	- text size larger 
# 	- naming: WT, MECR-KO (closed circles for both, blue for KO and white for WT)
# 	- strip sway 
# 	- use the some colors as in their figs|
...
# Renaming
pca_data$group <- factor(pca_data$group, levels = c("WT", "KO"))
levels(pca_data$group) <- c("WT", "MECR-KO")  # Rename levels

# Create the PCA plot with the specified properties
p <- ggplot(pca_data, aes(x = PC1, y = PC2, fill = group, color = group, shape = group)) +
  geom_point(size = 14, stroke = 1.2) +  # Increase point size and add contour
  scale_shape_manual(values = c(21, 21)) +  # Use circles for all points
  scale_color_manual(values = c("WT" = "black", "MECR-KO" = "black")) +  # Black contour for both groups
  scale_fill_manual(values = c("WT" = "white", "MECR-KO" = "blue")) +  # Fill color for groups
  labs(fill = "Condition", color = "Condition", shape = "Condition") +  # Set one legend for all
  xlab(paste0("PC1: ", round(percentVar[1], 1), "% variance")) +
  ylab(paste0("PC2: ", round(percentVar[2], 1), "% variance")) +
  ggtitle("Principal Component Analysis on the logCPM") +
  custom_minimal_theme_with_grid() +
  
  # Set custom axis limits and breaks
  scale_x_continuous(limits = c(-110, 110), breaks = c(-100, -50, 0, 50, 100)) +  # X-axis breaks
  scale_y_continuous(limits = c(-110, 110), breaks = c(-100, -50, 0, 50, 100)) +  # Y-axis breaks
  
  theme(
    # Inside positioning
    # Customize the grid to align with axis breaks
    panel.grid.major.x = element_line(color = "grey80", size = 0.5),  # Major grid lines on x-axis
    panel.grid.major.y = element_line(color = "grey80", size = 0.5),  # Major grid lines on y-axis
    ...
    # Adjust margins between title and axes
    axis.title.x = element_text(size = 24, margin = margin(t = 20)),  # Add top margin for x-axis title
    axis.title.y = element_text(size = 24, margin = margin(r = 10)),  # Add right margin for y-axis title
    
    # Adjust x and y axis text size (x-axis text slightly bigger)
    axis.text.x = element_text(size = 20),  # Increase x-axis text size
    axis.text.y = element_text(size = 20),  # Keep y-axis text size
    
    # Plot title
    plot.title = element_text(size = 20, hjust = 0.45, vjust = 0.5, margin = margin(b = 30)),  # Center title with margin
    plot.margin = margin(20, 40, 20, 20)  # Adjust margin around the plot
  )
...
ggsave(
  filename = "imgs/final_figs/PCA_plot.pdf",  # Specify the file name
  plot = p,  # The plot to save
  width = 8, height = 6,  # Adjusted width to accommodate the legend outside the plot
  dpi = 300  # Resolution of the PDF
)

# Save the plot as a SVG
ggsave(
  filename = "imgs/final_figs/PCA_plot.svg",  # Specify the file name
  plot = p,  # The plot to save
  width = 7, height = 5,  # Adjusted width to accommodate the legend outside the plot
  dpi = 300  # Resolution of the SVG
)
...
# Define a custom minimal theme without minor grid lines
custom_minimal_theme_with_grid <- function() {
  theme_minimal(base_size = 14) +
    theme(
      panel.background = element_rect(fill = "white", color = NA), 
      plot.background = element_rect(fill = "white", color = NA),
      panel.grid.major = element_line(color = "grey80", size = 0.5),  # Keep major grid lines
      panel.grid.minor = element_blank(),  # Remove minor grid lines
      axis.line = element_line(color = "black"),
      axis.ticks = element_line(color = "black"),
      axis.title.x = element_text(size = 14, face = "plain"),
...
create_volcano_plot <- function(de_results, p_cutoff = 0.05, fc_cutoff = 2.0, 
  ...
  # Prepare DE results and add hover text (although hover text will not be used in static plot)
  de_results <- de_results %>%
    mutate(
      significant_fc = abs(logFC) > fc_cutoff,
      significant_p = P.Value < p_cutoff,
...
# Example Usage:
# Assuming your DE data is stored in `DE_KOvsWT`
volcano_plot <- create_volcano_plot(DE_KOvsWT, p_cutoff = 0.05, 
                                    fc_cutoff = 4.0, 
                                    color_pallette = c("gray", "forestgreen", "skyblue", "orange"),
                                    x_breaks = 5
                                    )
...
# 2. Volcano plot Rasterize, non-interactive 
# - Log2FC 
# - -Log10P: color only p-value crossing dots:
# 	- negative Log2FC: red. Put how many dots are negative (post-adjustment)
# 	- positive Log2FC: blue. Put how many dots are positive (post-adjustment)
# 	- colorcode by FDR, but p-values for visualisation (FDR 0.05)
# 	- remove internal grid 
# 	- make as small as possible 
# 	- big text for axises 
# 	- sparse breaks on the x-axis 
...
custom_minimal_theme_with_grid <- function() {
  theme_minimal(base_size = 14) +
    theme(
      ...
      axis.line = element_line(color = "black"),
      axis.ticks = element_line(color = "black"),
      axis.title.x = element_text(size = 24, face = "plain", margin = margin(t = 20)),  # Adjusted size and margin
      axis.title.y = element_text(size = 24, face = "plain", margin = margin(r = 10)),  # Adjusted size and margin
      axis.text.x = element_text(size = 16),  # Adjust x-axis text size
      axis.text.y = element_text(size = 16)   # Adjust y-axis text size
    )
}

# Main function to generate a volcano plot with custom categories, adjusted text sizes, and annotations
create_volcano_plot <- function(de_results, p_cutoff = 0.05, fc_cutoff = 2.0, 
                                x_breaks = 1, color_pallette = c("gray", "red", "blue")) {

  # Prepare DE results and add significance columns for adj.P.Val
  de_results <- de_results %>%
    mutate(
      significant_fc = abs(logFC) > fc_cutoff,
      significant_p = adj.P.Val < p_cutoff,  # Use adj.P.Val for significance
      highlight_pos = significant_fc & significant_p & logFC > 0,
      highlight_neg = significant_fc & significant_p & logFC < 0
    ) %>%
    # Add a color column based on the condition
    mutate(
      color = case_when(
        highlight_pos ~ "p-value & Log2FC+",
        highlight_neg ~ "p-value & Log2FC-",
        TRUE ~ "NS"
      ),
      point_size = case_when(
        highlight_pos | highlight_neg ~ 3,  # Size 3 for significant points
        TRUE ~ 2                            # Size 2 for non-significant points
      )
    )
  
  # Count the number of positive and negative significant genes
  num_genes_pos <- sum(de_results$highlight_pos)
  num_genes_neg <- sum(de_results$highlight_neg)
  ...
  # Define base plot
  base_plot <- ggplot(de_results, aes(x = logFC, y = -log10(P.Value), color = color, size = point_size)) +
    geom_point(alpha = 0.5, stroke = 0.5) +  # Set alpha to 0.5 for transparency
    ...
    scale_color_manual(values = c("NS" = color_pallette[1], "p-value & Log2FC+" = color_pallette[2], "p-value & Log2FC-" = color_pallette[3]),
                       name = NULL  # Remove the legend title
    ) +
    scale_size_identity() +  # Use the size mapping from the data
    labs(
      title = "Volcano Plot",
      subtitle = paste("FDR cutoff:", p_cutoff, "| FC cutoff:", fc_cutoff),
      x = bquote(~Log[2]~ 'FC'),   # Subscript for Log2FC
      y = bquote(-Log[10]~ 'P')    # Subscript for Log10P
    ) +
    scale_x_continuous(breaks = seq(x_min, x_max, by = x_breaks), limits = c(x_min, x_max)) +
    custom_minimal_theme_with_grid() +
    theme(
      legend.position = c(0.6, 1.0),  # Inside positioning
      legend.justification = c(0, 1),  # Align legend to top-left inside plot
      legend.box.background = element_rect(color = "black", linewidth = 0.5),  # Black contour around legend box
      legend.box.margin = margin(1, 1, 1, 1),  # Adjust margin around legend box
      axis.title.x = element_text(size = 24, margin = margin(t = 10)),  # Add top margin for x-axis title
      axis.title.y = element_text(size = 24, margin = margin(r = 10))  # Add right margin for y-axis title
    ) +
    # Add annotations for the number of genes positive and negative
    annotate("text", x = 15, y = 2.5, label = paste(num_genes_pos, "positive"), color = "red", size = 5, hjust = 1) +
    annotate("text", x = -16, y = 2.5, label = paste(num_genes_neg, "negative"), color = "blue", size = 5, hjust = 0)
...
volcano_plot <- create_volcano_plot(DE_KOvsWT, p_cutoff = 0.05, 
                                    fc_cutoff = 4.0, 
                                    color_pallette = c("gray", "red", "blue"),
                                    x_breaks = 5
                                    )
...
ggsave(filename = "imgs/final_figs/volcano_plot2.pdf", 
       plot = volcano_plot, 
       width = 6, 
       height = 5, 
       dpi = 300)

ggsave(filename = "imgs/final_figs/volcano_plot2.svg", 
       plot = volcano_plot, 
       width = 6, 
       height = 5, 
       dpi = 300)
...
volcano_plot <- volcano_plot +
  scale_x_continuous(breaks = seq(-20, 16, by = 5), limits = c(-21, 16))
...
custom_dotplot <- function(gsea_obj, showCategory = 10, font.size = 10, title = "GSEA Dotplot",
                           ...
                           q_cut = 0.05,
                           min.dotSize = 2,
                           user_list_to_display = NULL,
                           user_list_names = NULL) {
  
  # Extract the result data frame from the GSEA object
  gsea_data <- as.data.frame(gsea_obj@result)
  
  # If user_list_to_display is provided, filter by the specified pathways
  if (!is.null(user_list_to_display)) {
    gsea_data <- gsea_data %>%
      filter(Description %in% user_list_to_display)
  }

  # Proceed with normal processing and apply capitalization if user_list_names is NULL
  if (is.null(user_list_names)) {
    # Modify Description field based on function arguments
    if (replace_) {
      gsea_data$Description <- gsea_data$Description %>% 
        str_replace_all("_", " ")   # Replace "_" with " " if 'replace' is TRUE
  ...
  # Filter for significant pathways (qvalue < 0.05 or as set by q_cut)
  gsea_data_filtered <- gsea_data %>%
    filter(qvalue < q_cut) %>%
    mutate(NES_sign = ifelse(NES > 0, "Positive NES", "Negative NES"))
  ...
  # Now assign user_list_names to the Description after all sorting and filtering
  if (!is.null(user_list_names)) {
    # Make sure user_list_names length matches the number of pathways in gsea_data_filtered
    if (length(user_list_names) != nrow(gsea_data_filtered)) {
      stop("user_list_names must have the same length as the filtered pathways.")
    }
    
    # Assign the user-provided names to the final filtered data
    gsea_data_filtered$Description <- user_list_names
  }
...
3. Enriched plot 
- Hallmark, top 20. 
- Remove dedundant pathways:
	- HALLMARK MYC TARGETS V2
	- HALLMARK ADIPOGENESIS
* Top 10 with removed 2 pathways: specifically say what has been removed 
* Text, first letter capital.
* Bubble plots from NASH lesson: big dots, big text 
* Narrow the plot as max as possible 
* "Negative NES" legend remove 
* Top 12 curated. 


```{r include = FALSE}
# Get the same order as in the internal plot function logic 
  # Gene Count calculation
   gene_count <- GSE_H %>%
    group_by(ID) %>%
    summarise(count = sum(str_count(core_enrichment, "/")) + 1)

 # GeneRatio calculation
   GSE_H_plot <- left_join(GSE_H[], gene_count, by = "ID") %>%
    mutate(GeneRatio = count / setSize)
  
  # Filter for qvalue < 0.05
   GSE_H_filter <- GSE_H_plot %>%
    filter(qvalue < 0.05)
     
  # Sort by GeneRatio as in the dotplot 
   GSE_H_filt_sort <- GSE_H_filter %>%
      arrange(desc(GeneRatio)) 

# Filter out redandant pathways like version 2 of Myc
Top12H_curated <- GSE_H_filt_sort[c(1:5, 7:13),]
```


```{r include = FALSE}

# Define custom lettering type for the pathways
Top12H_curated_names <- 
  c(
    "MYC targets",
    "Protein secretion",
...
p <- custom_dotplot(gsea_obj = GSE_H, 
               showCategory = 12, 
               font.size = 10, 
               title = "Top 12 MECR-KO enriched hallmark pathways, qvalue < 0.05",
               sortBy = "GeneRatio", 
               filterBy = "NES", 
               q_cut = 0.05,
               replace_ = TRUE, 
               ...
               capitalize_all = FALSE,
               min.dotSize = 3, 
               user_list_to_display = Top12H_curated$Description,
               user_list_names = Top12H_curated_names
               )

# 1. Prevent dots from being cut off using coord_cartesian or expand_limits
p <- p + 
  coord_cartesian(xlim = c(0.6, max(p$data$GeneRatio) * 1.05)) +  # Add 5% padding on the right for x-axis
  expand_limits(y = 1)  # Add padding to ensure y-labels aren't cut off

# 2. Remove the "NES" legend if only Negative NES pathways are present
p <- p + scale_color_manual(values = "blue") +  # Set all dots to blue
  guides(color = "none")  # Remove the color legend for NES

# 3. Adjust x title size, axis text sizes, and spacing
p <- p + theme(
  axis.title.x = element_text(size = 14, margin = margin(t = 10)),  # Make x title smaller and closer
...
# Save the plot to disk using ggsave()
ggsave(
  filename = "imgs/final_figs/GSEA/Hallmark_top12.pdf",  # Specify the file path and name
  plot = p,  # The plot to save
  width = 5.6, height = 5,  # Dimensions of the image in inches
  dpi = 300  # Resolution of the image
)
```
...
```{r include = FALSE}
# ### 4. Running sum plot - Hallmark
# * Keep the combine top 5 unchanged (for supllementary)

GSE_H@result %>%
  arrange(qvalue) %>%  # Sort by qvalue in ascending order
  slice_head(n = 5)    # Select the top 5 pathways
```
...
# Create the original plot using gseaplot2
p_list <- gseaplot2(GSE_H, geneSetID = 1:5, subplots = c(1, 2, 3))  # This returns a list of ggplot objects

# Modify each individual plot within the list
p_list[[1]] <- p_list[[1]] + 
  
    scale_color_manual(
    values = c("HALLMARK_E2F_TARGETS" = "#648FFF", 
               "HALLMARK_G2M_CHECKPOINT" = "#785EF0",
               "HALLMARK_MTORC1_SIGNALING" = "#DC267F",
               "HALLMARK_MYC_TARGETS_V1" = "#FE6100",
               "HALLMARK_OXIDATIVE_PHOSPHORYLATION" = "#FFB000"
               ),  # Define colors for your renamed labels
    labels = c("Hallmark E2F targers", 
               "Hallmark G2M checkpoint", 
               "Hallmark mTORC1 signaling", 
               "Hallmark MYC targets", 
               "Hallmark oxidative phosphorylation")  # Rename legend labels here
  ) +
  
  theme(
    # Adjust x and y axis title size
    axis.title.x = element_blank(),  # Make x title larger
    axis.title.y = element_text(size = 16),  # Make y title larger
    
    # Adjust x and y axis text size
    axis.text.x = element_blank(),  # Make x-axis labels larger
    axis.text.y = element_text(size = 14),  # Make y-axis labels larger
    
    # Adjust the legend position, shifting it 0.2 points to the left
    legend.position = c(0.65, 0.8),  # Move legend to left
...
p_list[[2]] <- p_list[[2]] + 
  ...
  theme(
   # Adjust x and y axis title size
    axis.title.x = element_blank(),  # Make x title larger
    axis.title.y = element_blank(),  # Make y title larger
    
    # Adjust x and y axis text size
    axis.text.x = element_blank(),  # Make x-axis labels larger
    axis.text.y = element_blank(),  # Make y-axis labels larger
  )
...
p_list[[3]] <- p_list[[3]] + 
  theme(
   # Adjust x and y axis title size
    axis.title.y = element_text(size = 16),  # Make y title larger
    axis.title.x = element_text(size = 16),  # Make y title larger
...
# Plot the modified first subplot
print(p_list)

# Save the modified plot to disk
ggsave(
  filename = "imgs/final_figs/RunningScore_top5_modified.png",  # Specify the file path and name
  plot = p_list,  # The modified first plot to save
  width = 8, height = 6,  # Dimensions of the image in inches
  dpi = 300  # Resolution of the image
)
...
# Save the modified plot to disk
ggsave(
  filename = "imgs/final_figs/RunningScore_top5_modified.pdf",  # Specify the file path and name
  plot = p_list,  # The modified first plot to save
  width = 8, height = 6,  # Dimensions of the image in inches
  dpi = 300  # Resolution of the image
)
...
```{r include = FALSE}
# ### Prepare & send Pathway data geneset lists, which we want to push through online tools
# 
# 
# #####################################
# ### Top 12 curated 
...
# 
# #####################################
# ### Hallmark Pathways: Top 12 curated 
# #####################################
# # First, split the core_enrichment genes for each pathway
# gene_lists <- strsplit(Top12H_curated$core_enrichment, "/")
# 
# # Create a named list where each entry corresponds to the Description of the pathway
# named_gene_lists <- setNames(gene_lists, Top12H_curated$Description)
# 
# # Find the maximum number of genes in any gene set
# max_genes <- max(sapply(named_gene_lists, length))
# 
# # Create a data frame where each column is a gene set and each row is a gene
# gene_matrix <- as.data.frame(sapply(named_gene_lists, function(genes) {
#   # Pad shorter lists with NA so they all have the same length
#   length(genes) <- max_genes
#   return(genes)
# }))
# 
# # Add the new column to the existing gene_matrix
# gene_matrix$GOMF_ELECTRON_TRANSFER_ACTIVITY <- selected_genes
# 
# #####################################
# ### GSE GO:MF selected pathway
# #####################################
# # Select the pathway "GOMF_ELECTRON_TRANSFER_ACTIVITY" from GSE_GO
# selected_pathway <- GSE_GO %>%
#   filter(Description == "GOMF_ELECTRON_TRANSFER_ACTIVITY") %>%
#   select(Description, core_enrichment)
# 
# # Electron transfer split
# selected_genes <- strsplit(selected_pathway$core_enrichment, "/")[[1]]
# 
# # Get the number of rows in the existing gene_matrix
# n_existing_rows <- nrow(gene_matrix)
# 
# # Ensure both vectors have the same length by filling the shorter one with NA
# selected_genes <- c(selected_genes, rep(NA, n_existing_rows - length(selected_genes)))
# 
# # Add the new column to the existing gene_matrix
# gene_matrix$GOMF_ELECTRON_TRANSFER_ACTIVITY <- selected_genes
# 
# # If you need to fill any remaining rows with NA
# n <- max(nrow(gene_matrix), length(selected_genes))
# gene_matrix <- mutate_all(gene_matrix, list(~c(., rep(NA, n - length(.)))))
# 
# 
# # Save the updated gene_matrix to a CSV file
# #write.csv(gene_matrix, file = "SelectGeneSets_GeneMatrix.csv", row.names = TRUE)
```
...
# Function to generate citations for each package
get_citations <- function(packages) {
  lapply(packages, function(pkg) {
    tryCatch(
      citation(pkg)$textVersion, 
      error = function(e) paste("No citation information for", pkg)
    )
  })
}

# Get the package names
package_names <- names(loaded_packages)

# Generate citations
citations <- get_citations(package_names)

# Print the citations
cat(unlist(citations), sep = "\n\n")
