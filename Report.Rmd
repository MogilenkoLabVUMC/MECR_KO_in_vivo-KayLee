...
```{r}
#### Another normalisation approach 
...
```{r}
##################
### Combined Volcano Interactive: Claude with GPT Plotly Hover
##################
  ...
  de_results <- de_results %>%
    mutate(
      ...
      hover_info = paste("Gene:", rownames(.),
                         ...
                         "<br>Adj.P.Value:", round(adj.P.Val, 5))  # Hover text with gene info
    ) %>%
    # Add a color column based on the condition
    mutate(
      color = case_when(
...
# Call this function with your data to create the interactive plot.
...
[11] "HALLMARK_PI3K_AKT_MTOR_SIGNALING" 
[35] "HALLMARK_IL6_JAK_STAT3_SIGNALING"   
[47] "HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION"
...
#### TCR running score plot 
```{r}
title = "KEGG T CELL RECEPTOR SIGNALING PATHWAY"
...
# Save the plot to disk using ggsave()
ggsave(
  filename = "imgs/GSEA/KEGG/RunningScore_TCR.png",  # Specify the file path and name
  plot = p,  # The plot to save
  width = 10, height = 5,  # Dimensions of the image in inches
  dpi = 300  # Resolution of the image
)
...
## 4.4 IMMUNESIGDB Pathways enrichment analysis
